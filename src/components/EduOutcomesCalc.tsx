...
const NAVY = '#1e3a7b';
const GOOD = '#1b7a43'; // a rise is positive in education
const BAD = '#b0233a';  // a decline
const f1 = (v: number) => v.toFixed(1);
const deltaColor = (d: number) => d > 0 ? GOOD : d < 0 ? BAD : '#9ca3af';
const deltaText = (d: number) => `${d > 0 ? '+' : d < 0 ? '−' : ''}${Math.abs(d).toFixed(1)}`;
...
const pillBtn = (active: boolean): React.CSSProperties => ({
  padding: '6px 14px', borderRadius: 16, border: `1px solid ${active ? NAVY : '#e5e7eb'}`,
  background: active ? NAVY : '#fff', color: active ? '#fff' : '#6b7280',
  fontFamily: 'inherit', fontSize: 12.5, fontWeight: active ? 700 : 500, cursor: 'pointer',
});
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', marginBottom: 20 }}>
          ...
          <div style={{ padding: '16px 18px 12px' }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#111827', marginBottom: 3 }}>
              הישגים בחינוך — השוואת 2022 ו-2024
            </div>
          ...
          <div style={{ display: 'flex', gap: 6, padding: '0 16px 12px' }}>
            ...
            <button type="button" onClick={() => setView('clusters')} style={segBtn(view === 'clusters')}>לפי אשכול</button>
          ...
          <div style={{ display: 'flex', gap: 6, padding: '0 16px 12px', flexWrap: 'wrap', borderBottom: '1px solid #e5e7eb' }}>
            {EDU_METRICS.map(m => (
              <button key={m.key} type="button" onClick={() => setMetricKey(m.key)} style={pillBtn(m.key === metricKey)}>
                {m.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ padding: '10px 18px', borderTop: '1px solid #f3f4f6', background: '#fafbfc', fontSize: 10, color: '#9ca3af', lineHeight: 1.7 }}>
            {view === 'clusters'
              ...
              : view === 'national'
              ? 'נתונים ארציים רשמיים לפי שנת לימודים. '
              : 'האשכול הסוציו-אקונומי (1–10) מאפשר השוואה בין רשויות במעמד חברתי-כלכלי דומה. '}
            מקור: משרד החינוך — דוח תמונה חינוכית רשותי / שקיפות בחינוך, 2022 ו-2024.
          </div>
        </div>
      </div>
    </div>
...
function ClusterTag({ c }: { c: number | null }) {
  ...
  return <span style={{ fontSize: 9.5, fontWeight: 600, color: '#475569', background: '#eef2f6', borderRadius: 3, padding: '1px 6px', whiteSpace: 'nowrap' }}>אשכול {c}</span>;
}
  ...
  return (
    <>
      ...
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #e5e7eb' }}>
        <div style={{ fontSize: 10.5, fontWeight: 600, color: '#6b7280', marginBottom: 8 }}>
          ...
          {(['all', ...CLUSTERS] as const).map(c => {
            ...
            return (
              <button key={String(c)} type="button" onClick={() => setCluster(c as number | 'all')}
                style={{ padding: '4px 11px', borderRadius: 14, border: `1px solid ${on ? NAVY : '#e5e7eb'}`, background: on ? NAVY : '#fff', color: on ? '#fff' : '#6b7280', fontFamily: 'inherit', fontSize: 12, fontWeight: on ? 700 : 500, cursor: 'pointer', minWidth: c === 'all' ? 'auto' : 30 }}>
                {c === 'all' ? 'הכל' : c}
              </button>
            );
          })}
        ...
        {query.trim() ? (
          searchResults.length === 0 ? (
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: 6, overflow: 'hidden' }}>
              {searchResults.map((r, idx) => {
                ...
                return (
                  <div key={r.name} style={{ padding: '12px 14px', borderBottom: idx < searchResults.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, direction: 'ltr', fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>
                        ...
                        <span style={{ fontSize: 16, color: NAVY, fontWeight: 800 }}>{b != null ? `${f1(b)}%` : '—'}</span>
                        {diff != null && <span style={{ fontSize: 12.5, fontWeight: 700, color: deltaColor(diff) }}>{deltaText(diff)}</span>}
                      </div>
                    </div>
                  </div>
                );
          ...
          <>
            <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
              <button type="button" onClick={() => setMode('drops')} style={segBtn(mode === 'drops')}>ירידות חדות</button>
              <button type="button" onClick={() => setMode('rises')} style={segBtn(mode === 'rises')}>עליות בולטות</button>
            </div>
            ...
            {movers.length === 0 ? (
              ...
              <div style={{ border: '1px solid #e5e7eb', borderRadius: 6, overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '20px 1fr 52px 52px 56px', gap: 8, padding: '7px 14px', background: '#f9fafb', borderBottom: '1px solid #e5e7eb', fontSize: 10.5, color: '#9ca3af', fontWeight: 600 }}>
                ...
                {movers.map((r, i) => (
                  <div key={r.name} style={{ display: 'grid', gridTemplateColumns: '20px 1fr 52px 52px 56px', alignItems: 'center', gap: 8, padding: '10px 14px', borderBottom: i < movers.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                    ...
                    <span style={{ fontSize: 13, color: '#111827', fontWeight: 700, textAlign: 'center', direction: 'ltr', fontVariantNumeric: 'tabular-nums' }}>{f1(r.y2024)}</span>
                    <span style={{ fontSize: 12.5, fontWeight: 700, color: deltaColor(r.diff), textAlign: 'left', direction: 'ltr', fontVariantNumeric: 'tabular-nums' }}>{deltaText(r.diff)}</span>
                  </div>
                ))}
              </div>
            )}
...
function ClustersView({ metricKey, metricLabel }: { metricKey: EduMetricKey; metricLabel: string }) {
  ...
  return (
    <div style={{ padding: '16px 18px' }}>
      ...
      <div style={{ fontSize: 10.5, color: '#9ca3af', marginBottom: 14 }}>
        ממוצע משוקלל לכל אשכול, 2022 ו-2024.{nat2024 != null && <> הקו המקווקו = הנתון הארצי 2024 ({f1(nat2024)}%).</>}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {rows.map(r => (
          <div key={r.cluster} style={{ display: 'grid', gridTemplateColumns: '64px 1fr 92px', alignItems: 'center', gap: 10, padding: '8px 4px', borderBottom: '1px solid #f6f7f8' }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#374151' }}>אשכול {r.cluster}</div>
            ...
            <div style={{ position: 'relative', height: 22 }}>
              ...
              <div style={{ position: 'absolute', top: 10, right: 0, height: 6, width: `${r.y2024}%`, background: NAVY, borderRadius: 3 }} />
              {r.y2022 != null && (
                <div title={`2022: ${f1(r.y2022)}%`} style={{ position: 'absolute', top: 6, right: `${r.y2022}%`, width: 2, height: 14, background: '#94a3b8', transform: 'translateX(50%)' }} />
              )}
              {nat2024 != null && (
                <div style={{ position: 'absolute', top: 2, right: `${nat2024}%`, width: 0, height: 18, borderRight: '1.5px dashed #94a3b8', transform: 'translateX(50%)' }} />
              )}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, direction: 'ltr', justifyContent: 'flex-end', fontVariantNumeric: 'tabular-nums' }}>
              <span style={{ fontSize: 14, fontWeight: 800, color: NAVY }}>{f1(r.y2024)}%</span>
              {r.diff != null && <span style={{ fontSize: 11.5, fontWeight: 700, color: deltaColor(r.diff) }}>{deltaText(r.diff)}</span>}
            </div>
          </div>
        ))}
      </div>
...
function NationalView({ metricKey, metricLabel }: { metricKey: EduMetricKey; metricLabel: string }) {
  ...
  return (
    <div style={{ padding: '16px 18px' }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: NAVY, marginBottom: 2 }}>מגמה ארצית — {metricLabel}</div>
      <div style={{ fontSize: 10.5, color: '#9ca3af', marginBottom: 14 }}>
        נתון ארצי רשמי לפי שנת לימודים, {series[0]?.label} עד {series[series.length - 1]?.label}.
      </div>
        ...
        {series.map((s, i) => {
          ...
          return (
            <div key={s.year} style={{ display: 'grid', gridTemplateColumns: '54px 1fr 86px', alignItems: 'center', gap: 10, padding: '5px 2px' }}>
              <span style={{ fontSize: 11.5, color: isLast ? NAVY : '#6b7280', fontWeight: isLast ? 800 : 500 }}>{s.label}</span>
              <div style={{ height: 16, background: '#f1f3f5', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(s.v / max) * 100}%`, background: isLast ? NAVY : '#9fb0cf', borderRadius: 3 }} />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, direction: 'ltr', justifyContent: 'flex-end', fontVariantNumeric: 'tabular-nums' }}>
                <span style={{ fontSize: 13, fontWeight: isLast ? 800 : 600, color: isLast ? NAVY : '#374151' }}>{f1(s.v)}%</span>
                {d != null && <span style={{ fontSize: 10.5, fontWeight: 600, color: deltaColor(d) }}>{deltaText(d)}</span>}
              </div>
            </div>
          );
        })}

...
export default function EduBudgetCalc() {
  ...
  const lowest = EDU_BUDGET.slice(0, 5);                  // 5 lowest-funded
  const highest = EDU_BUDGET.slice(-5).slice().reverse(); // 5 highest-funded

  const results = useMemo(() => {
    const q = query.trim();
    if (!q) return [];
    return EDU_BUDGET
      .map((x, i) => ({ ...x, rankHigh: total - i })) // 1 = highest funded
      .filter(x => x.name.includes(q))
      .sort((a, b) => b.perStudent - a.perStudent)
      .slice(0, 40);
  }, [query, total]);

  function Row({ name, perStudent, n, color }: { name: string; perStudent: number; n: number; color: string }) {
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '22px 1fr 78px', alignItems: 'center', gap: 8, padding: '9px 12px', borderBottom: '1px solid #f3f4f6' }}>
        <span style={{ fontSize: 11, color: '#d1d5db', textAlign: 'center', fontVariantNumeric: 'tabular-nums' }}>{n}</span>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ marginTop: 4, height: 3, background: '#f3f4f6', borderRadius: 2, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${(perStudent / maxPer) * 100}%`, background: color, borderRadius: 2 }} />
          </div>
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', marginBottom: 20 }}>
            ...
            {query.trim() && (
              <div style={{ marginTop: 12, border: '1px solid #e5e7eb', borderRadius: 6, overflow: 'hidden' }}>
                {results.length === 0 ? (
                  ...
                  return (
                    <div key={r.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '11px 14px', borderBottom: '1px solid #f3f4f6' }}>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: '#111827' }}>{r.name}</div>
                        <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>
                          מקום {r.rankHigh} מתוך {total} · {above ? 'מעל' : 'מתחת ל'}חציון ({Math.abs(diff)}%{above ? '+' : '−'})
                        </div>
                      </div>
                      <span style={{ fontSize: 15, fontWeight: 800, color: ACCENT, direction: 'ltr', fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>
                        {fmt(r.perStudent)}₪
                      </span>
                    </div>
                  );
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 0, borderTop: '1px solid #e5e7eb' }}>
            ...
            <div style={{ borderLeft: '1px solid #e5e7eb' }}>
              <div style={{ padding: '12px 14px 8px', fontSize: 12, fontWeight: 700, color: '#b91c1c' }}>
                5 הרשויות עם התקצוב הנמוך ביותר
              </div>
              {lowest.map((x, i) => <Row key={x.name} name={x.name} perStudent={x.perStudent} n={i + 1} color="#c2410c" />)}
            </div>
            {/* Highest */}
            <div>
              <div style={{ padding: '12px 14px 8px', fontSize: 12, fontWeight: 700, color: '#1e3a7b' }}>
                5 הרשויות עם התקצוב הגבוה ביותר
              </div>
              {highest.map((x, i) => <Row key={x.name} name={x.name} perStudent={x.perStudent} n={i + 1} color="#1e3a7b" />)}
            </div>
          </div>
